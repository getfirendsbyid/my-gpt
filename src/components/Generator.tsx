    ...
    try {
      ...
      while (!done) {
        ...
        if (value) {
          ...
          if (char) {
            ++total;
            const currentTotal = storage.getCookie("total");
            if (currentTotal === "") {
              storage.setCookie("total", 0);
              console.log("currentTotal", currentTotal);
            }
            storage.setCookie("total", parseInt(currentTotal) + 1);

            console.log("total", total);

            console.log(total);
            setCurrentAssistantMessage(currentAssistantMessage() + char);
            console.log("currentAssistantMessage", char);
          }
          smoothToBottom();
        }
        done = readerDone;
